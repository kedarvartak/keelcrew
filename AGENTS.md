...
### claude — architect
**claude** — Project rebranded: multi-agent-memo is now KEELCREW (npm name free, GitHub repo renamed to kedarvartak/keelcrew, old URLs redirect). #decision Product pivot: single-terminal CLI harness (no GUI) that spawns Claude/Codex/Gemini as headless workers on one checkout, with directed agent-to-agent messaging (questions/replies/reviews, threaded) rendered as a live crosstalk feed beside the board and per-agent streams. Full PM plan in docs/plan.md (phases P1-P5 with acceptance criteria, risks, non-goals); diagrams in docs/diagrams/ (architecture, session-flow, roadmap). README rewritten, no emojis, banner removed. Local folder is still named multi-agent-memo — rename it manually when convenient. #todo Phase 1 next: messages.ts + keelcrew watch.

### claude — architect
**claude** — Phase 1 shipped and committed (d6bfb3c on manual-writedown-memory; rebrand commit 75e70d3). New: src/messages.ts (directed threaded messages, question/info/review kinds, blocking questions, per-agent cursors, open-question tracking), MCP tools send_message/get_messages, get_context crosstalk section, and the keelcrew CLI: watch (live dashboard), board, log --follow, say, mcp. src/index.ts renamed to src/mcp.ts; bin is now dist/cli.js so MCP configs must use args ["keelcrew","mcp"]. 26/26 tests. #decision Next per docs/plan.md: Phase 2 — claude adapter (stream-json) + single headless worker loop. #todo
