...
| P1 | Directed agent messaging (threads, questions, captain) and the CLI skeleton: `keelcrew watch\|board\|log\|say\|mcp` | Shipped |
| P2 | Adapters for claude/codex/gemini and the single-worker headless loop (`keelcrew run`) | Shipped |
...
| P6 | Interactive conductor console: directed assignment, `delegate_task`, keep-alive session, conductor, `keelcrew` REPL, `crew` | Shipped |
...
- The v0.1 writedown layer (session snapshots, generated `AGENTS.md` index)
  survives unchanged as keelcrew's memory subsystem.
- The web UI / tunnel concept in `idea.md` is retired; keelcrew is
